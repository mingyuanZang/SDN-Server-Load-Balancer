...
public class AppComponent {
    ...
    protected void activate() {
        appId = coreService.registerApplication("org.student.lbWRR");
            ...
            */
            ...
            if (ipv4Packet.getProtocol() == IPv4.PROTOCOL_TCP) {
                ...
                if(srcPort == 0){
                    ...
                    currentWeight.add(0);
                    currentWeight.add(0);
                    log.info("Initial weight: " + weight);
                }
            ...
            /*
             * Weight RR select target server
             */
            int index = 0;
            for (int i=0; i<3; i++) {
                int weight_temp = currentWeight.get(i) + weight.get(i);
                currentWeight.set(i, weight_temp);
            }
            log.info("current weight: " + currentWeight);
            int weight_max = Collections.max(currentWeight);
            for (int i=0; i<3; i++){
                if(currentWeight.get(i) == weight_max){
                    index = i;
                }
            }
            currentWeight.set(index, weight_max-6);
            log.info("Updtae current weight: " + currentWeight);
            if(index == 0){
                serverMac = MacAddress.valueOf(server1_mac);
            } else if (index == 1){
                serverMac = MacAddress.valueOf(server2_mac);
            } else {
                serverMac = MacAddress.valueOf(server3_mac);
            }
