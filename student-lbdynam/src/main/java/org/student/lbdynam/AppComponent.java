...
public class AppComponent {
    ...
    protected void activate() {
        appId = coreService.registerApplication("org.student.lbdynam");
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            */
             ...
             * Select a server
             */
            ConcurrentHashMap<DeviceId, PortNumber> minDevicePort = outPortSelection(serverLookupTable, serverMac);
            for (Map.Entry<MacAddress, ConcurrentHashMap<DeviceId, PortNumber>> entry : serverLookupTable.entrySet()) {
                if(minDevicePort.equals(entry.getValue())){
                    serverMac = entry.getKey();
                }
            }
            ConcurrentHashMap.KeySetView<DeviceId, PortNumber> mindeviceset = minDevicePort.keySet();
            for (DeviceId minD:mindeviceset) {
                dstDevice = minD;
                outPort = minDevicePort.get(minD);
            }
    ...
    private  ConcurrentHashMap<DeviceId, PortNumber> outPortSelection(ConcurrentHashMap<MacAddress, ConcurrentHashMap<DeviceId,PortNumber>> lookupTable, MacAddress servMac){
        long portstat_min = 0;
        MacAddress macIndex = null;
        ConcurrentHashMap<MacAddress, Long> portStasTable = new ConcurrentHashMap();
        ConcurrentHashMap.KeySetView<MacAddress, ConcurrentHashMap<DeviceId, PortNumber>> set = lookupTable.keySet();
        for (MacAddress mac:set) {
            ConcurrentHashMap<DeviceId, PortNumber> deviceTable = lookupTable.get(mac);
            ConcurrentHashMap.KeySetView<DeviceId, PortNumber> deviceset = deviceTable.keySet();
            for (DeviceId d:deviceset) {
                PortNumber port = deviceTable.get(d);
                PortStatistics portstat = deviceService.getDeltaStatisticsForPort(d, port);
                long portstat_temp = portstat.packetsSent();
                portStasTable.put(mac, portstat_temp);
            }
        }
        log.info("Port statistics:" + String.valueOf(portStasTable));
        Collection<Long> statSet = portStasTable.values();
        portstat_min = Collections.min(statSet);
        log.info("Minimum packet sent:" + String.valueOf(portstat_min));
        ConcurrentHashMap.KeySetView<MacAddress, Long> macSet = portStasTable.keySet();
        for (MacAddress m:macSet){
            if(portStasTable.get(m).equals(portstat_min)){
                macIndex = m;
                if(macIndex.equals(servMac)){
                    log.info("Target server repeat");
                    portStasTable.remove(macIndex, portstat_min);
                    if(portStasTable.containsValue(portstat_min)){
                        log.info("Equal minimum values");
                        macSet = portStasTable.keySet();
                        for (MacAddress newm:macSet){
                            if(portStasTable.get(newm).equals(portstat_min)){
                                macIndex = newm;
                                log.info("Change to another server to avoid repeat: " + macIndex);
                            }
                        }
        ...
        ConcurrentHashMap<DeviceId, PortNumber> minDevicePort = lookupTable.get(macIndex);
        return minDevicePort;
    }
    ...
    private Path findBestPath(DeviceId srcDevice, DeviceId dstDevice){
        ...
        if(pathSet.isEmpty()) return null;
            ...
            if(pathSet.size() == 1){
                ...
                ConcurrentHashMap<Path, Long> pathRcvBytes = new ConcurrentHashMap<>();
                for (Path p : pathSet) {
                    long rcvBytes_max = 0;
                    log.info("path is: " + String.valueOf(p.links()));
                    List<Link> links = p.links();
                    for (Link l : links) {
                        PortStatistics dstPortStats = deviceService.getStatisticsForPort(l.dst().deviceId(), l.dst().port());
                    ...
                    pathRcvBytes.put(p, rcvBytes_max);
                }
                log.info("path received bytes list: " + String.valueOf(pathRcvBytes));
                Long rcvBytesList_min = Collections.min(pathRcvBytes.values());
                log.info("Minimum path received bytes: " + String.valueOf(rcvBytesList_min));
                ConcurrentHashMap.KeySetView<Path, Long> keys = pathRcvBytes.keySet();
                for(Path k:keys){
                    if(pathRcvBytes.get(k).equals(rcvBytesList_min)){
                        selectedPath = k;
                    }
                }
